...
List mode only lists playable cards (no tokens shall be listed).\n\
Min/MaxCardID (TF1) = 4007/6958\n\
Min/MaxCardID (TF3) = 3720/7994\n"
...
#define TFCD_HELPMESSAGE_VARS argv[0], argv[0], argv[0]
...
uint16_t GetInternalID(uint16_t CardID, uint16_t MinCardID)
{
    if (CardID < MinCardID)
        return 0;
...
int LoadIntIDs(const char* IntIDFilename)
{
    ...
    IntIDs = (uint16_t*)malloc(st.st_size);
...
int CreateIntIDs(const char* IntIDFilename)
{
    ...
    IntIDs = (uint16_t*)malloc(IntIDsize);
...
int LoadCardPasswords(const char* PassFilename)
{
    ...
    CardPasswords = (uint32_t*)malloc(st.st_size);
...
{
    CardPasswords = (uint32_t*)calloc(ImportedCardsCount + 1, sizeof(uint32_t));
    ...
    fwrite(CardPasswords, sizeof(uint32_t), ImportedCardsCount + 1, fpass);
...
uint32_t CARD_GetPassword(unsigned short int CardID)
{
...
int LoadFiles()
{
    ...
    if (bFileExists(FilePath))
        ...
        CardPasswords = (uint32_t*)calloc(IntIDCount, sizeof(int32_t));
...
int ListCardIDs(int inMinCardID, int inMaxCardID, char* outFilename)
{
    MinCard_ID = inMinCardID;
    MinCard_IntID = inMinCardID;
    MaxCard_ID = inMaxCardID;
...
int main(int argc, char* argv[])
{
    ...
    if (argv[1][0] == '-' && argv[1][1] == 'l')
    {
        ...
        if (argc == 5)
        {
            ...
            strcpy(OutTextName, argv[5]);

        return ListCardIDs(stol(argv[3]), stol(argv[4]), argv[5]);
    }
